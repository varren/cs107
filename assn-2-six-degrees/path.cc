...
void path::print() {
  cout<<"\"" << startPlayer <<"\" and \""<< links[0].player <<  "\" costared in film: \""<<links[0].movie.title<<"\" - "<< links[0].movie.year <<endl ; 
  for(u_int i=1;i<links.size();i++)
      
  cout<<"\"" << links[i-1].player <<"\" and \""<< links[i].player <<  "\" costared in film: \""<<links[i].movie.title<<"\" - "<< links[i].movie.year <<endl ;
