...
static const char *const kDefaultFeedsFile  = "/usr/class/cs107/assignments/assn-4-rss-news-search-data/rss-feeds-tiny-dup.txt";
...
//static const char *const kLinkTagPrefix = "<guid isPermaLink=\"false\""; 
...
static void ParseArticle(const char *articleTitle, const char *articleDescription, const char *articleURL)
{
  ...
  URLConnectionNew(&urlconn, &u);
  printf("\"%s\" from \"%s\". [Response code: %d] Punting...\n", articleTitle, u.fullName, urlconn.responseCode);
  switch (urlconn.responseCode) {
